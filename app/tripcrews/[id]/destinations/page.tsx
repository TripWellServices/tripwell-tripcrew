  ...
  async function handleGenerate(persist: boolean) {
    ...
    try {
      ...
      if (res.status === 409 && data.guide?.citySlug) {
        router.push(`/tripcrews/${tripCrewId}/destinations/${data.guide.citySlug}`)
        return
      }
      if (!res.ok) {
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-8 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          ...
          <input
            ...
            className="w-full sm:w-36 border border-gray-300 rounded-lg px-3 py-2 text-sm"
          />
        </div>
        {error && (
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {guides.map((g) => (
            <li key={g.id}>
              <Link
                href={`/tripcrews/${tripCrewId}/destinations/${g.citySlug}`}
