  ...
  if (error || !guide) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-10 space-y-3">
        ...
        <Link href={`/tripcrews/${tripCrewId}/destinations`} className="text-sky-600 text-sm hover:underline">
          ← All destinations
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Link
          href={`/tripcrews/${tripCrewId}/plan/destination?mode=trip&citySlug=${encodeURIComponent(guide.citySlug)}`}
          className="inline-flex px-4 py-2 rounded-lg bg-sky-600 text-white text-sm font-medium hover:bg-sky-700"
        >
          Plan a trip
        </Link>
        <Link
          href={`/tripcrews/${tripCrewId}/plan/destination?mode=season&citySlug=${encodeURIComponent(guide.citySlug)}`}
